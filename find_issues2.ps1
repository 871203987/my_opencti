$files = @(
    ...
    'd:\project\open_cti\opencti\opencti-java\src\main\java\io\opencti\database\stix\StixCoreRelationshipsMapping.java'
)
...
foreach ($file in $files) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        $line = $lines[$i]
        # Check if line has // followed by code on same line (not just another //)
